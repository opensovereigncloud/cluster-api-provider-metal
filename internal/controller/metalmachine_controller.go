...
func (r *MetalMachineReconciler) applyIgnitionSecret(ctx context.Context, log *logr.Logger, metalmachine *infrav1alpha1.MetalMachine, capidatasecret *corev1.Secret) (*corev1.Secret, error) {
	...
	if err := findAndReplaceIgnition(metalmachine, dataSecret); err != nil {
		return nil, fmt.Errorf("failed to set metal ignition changes: %w", err)
	}
...
func findAndReplaceIgnition(metalmachine *infrav1alpha1.MetalMachine, capidatasecret *corev1.Secret) error {
